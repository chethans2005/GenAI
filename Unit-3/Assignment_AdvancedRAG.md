...
For "How do transformers encode meaning?", Naïve RAG returned the generic transformer self-attention document, while Advanced RAG returned the more specific BERT document. This was the only query where the top document changed, showing that hybrid retrieval plus re-ranking can surface a more targeted match for ambiguous queries.

For "What is backpropagation and optimization?" and "Explain activation functions and regularization techniques.", both pipelines selected the same top document. These were direct terminology matches, so SBERT-only retrieval was already strong and the advanced pipeline did not need to change the final choice.

The comparison therefore showed that Advanced RAG adds the most value when the query is semantically vague or can map to multiple related documents.

## Conclusion

This assignment demonstrated that a modular Advanced RAG pipeline can improve retrieval quality over a dense-only baseline. The strongest gain came from combining BM25 and SBERT with RRF, then using a cross-encoder to refine the final ranking.

The experiment results support three conclusions. First, hybrid retrieval is more robust than SBERT alone because it captures both keyword overlap and semantic similarity. Second, cross-encoder re-ranking helps resolve ambiguous cases by scoring query-document pairs more precisely. Third, advanced retrieval is most useful for vague student-style questions, while direct factual queries can already be handled well by a simpler dense retriever.

HyDE was implemented in the notebook, but the executed comparison used hybrid retrieval plus re-ranking because of API quota limits. Even with that limitation, the final pipeline still produced better behavior on the ambiguous transformer query and remained stable on the direct-match queries.

Overall, the notebook meets the assignment goal: it shows a working Advanced RAG system, a comparison against Naïve RAG, and evidence-based results that justify the use of hybrid retrieval and re-ranking in production.
